...
public class ClientAuthenticationMeansCleanupServiceTest {
    ...
    private ClientAuthenticationMeansCleanupService subject;
    ...
    public void setup() {
        ...
        subject = new ClientAuthenticationMeansCleanupService(clock, providerFactory, clientRedirectUrlClientConfigurationRepository,
                meansEventDispatcherService, clientAuthenticationMeansRepository, authenticationMeansEncryptionService, authenticationMeansMapperService);
    }
    ...
    public void shouldCleanupProviderAuthenticationMeans() throws JsonProcessingException {
        ...
        Optional<InternalClientRedirectUrlClientConfiguration> authMeansFromDB = Optional.of(new InternalClientRedirectUrlClientConfiguration(
                CLIENT_ID, REDIRECT_URL_ID, SERVICE_TYPE, BUDGET_INSIGHT_NAME, encryptedAuthenticationMeans, Instant.now()));
        when(clientRedirectUrlClientConfigurationRepository.get(CLIENT_ID, REDIRECT_URL_ID, SERVICE_TYPE, BUDGET_INSIGHT_NAME)).thenReturn(authMeansFromDB);
        ...
        //when
        subject.cleanupProviderAuthenticationMeans(clientToken, BUDGET_INSIGHT_NAME, Collections.singleton(REDIRECT_URL_ID),
                Collections.singleton(SERVICE_TYPE), false);

        //then
        verify(clientRedirectUrlClientConfigurationRepository).upsert(storedAuthMeansRedirectUrl.capture());
    ...
    public void shouldOnlyLogProviderAuthenticationMeansToRemove() throws JsonProcessingException {
        ...
        //then
        verify(clientRedirectUrlClientConfigurationRepository, times(0)).upsert(any());
